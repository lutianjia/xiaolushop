   ...
   <div class="type_title">添加商品</div>
	<form action="" method="post" class="form form-horizontal" id="form-article-add">
        ...
        <div class="clearfix cl">
		 ...
		 <div class="formControls col-10"><input type="text" class="input-text" value="" placeholder="" id="product_name" name="product_name"></div>
		</div>

		<div class=" clearfix cl">
			<div class="Add_p_s">
             <label class="form-label col-2">产&nbsp;&nbsp;&nbsp;&nbsp;地：</label>	
			 <div class="formControls col-2"><input type="text" class="input-text" value="" placeholder="" id="country" name="country"></div>
			</div>
            <div class="Add_p_s">
             <label class="form-label col-2">品&nbsp;&nbsp;&nbsp;&nbsp;牌：</label>	
			 <div class="formControls col-2">
                 <select style="width:100px" name="brand" id="brand">
			 ...
			 <div class="formControls col-2"><input type="text" class="input-text" value="" placeholder="" id="weight" name="weight" >kg</div>
			</div>

            <div class="Add_p_s">
             <label class="form-label col-2">展示价格：</label>	
			 <div class="formControls col-2"><input type="text" class="input-text" value="" placeholder="" id="current_price" name="current_price" >元</div>
			</div>
            <div class="Add_p_s">
             <label class="form-label col-2">市场价格：</label>	
			 <div class="formControls col-2"><input type="text" class="input-text" value="" placeholder="" id="original_price" name="original_price" >元</div>
			</div>
		</div>
		<div class="clearfix cl">
        ...
        <div class="clearfix cl">
         <label class="form-label col-2">允许评论：</label>
			<div class="formControls col-2 skin-minimal">
			 <div class="check-box" style=" margin-top:9px"><input type="checkbox" id="checkbox-1" name="is_comment"><label for="checkbox-1">&nbsp;</label></div>
             </div>
        </div>
		<div class="clearfix cl">
			<div class="Button_operation">
				<s id="submit"><button class="btn btn-primary radius" type="button" onClick="article_save_submit('from_id');"><i class="icon-save "></i>保存并提交审核</button></s>
...
<script>
function article_save_submit(from_id){
	if(from_id == 'from_id'){
		layer.msg('请选择分类类型!',{icon:2,time:1000});
	}else{
		var str = "";
		var product_name = $('input[name=product_name]').val();
		if(product_name.length == 0) {
			layer.alert(str += "" + "商品名称" + "不能为空！\r\n", {
				title: '提示框',
				icon: 0,
			});
			return;
		...
		if(current_price.length == 0) {
			layer.alert(str += "" + "展示价格" + "不能为空！\r\n", {
				title: '提示框',
				icon: 0,
			});
			return;
		}
		var obj=document.getElementById("ueditor_0").contentWindow;
		var detail = obj.document.getElementsByTagName("body")[0].innerHTML;
		console.log(detail);
		if(detail.length == 0) {
			layer.alert(str += "" + "详细内容" + "不能为空！\r\n", {
				title: '提示框',
				icon: 0,
			});
			return;
        ...
        var is_comment=document.getElementById("checkbox-1").parentElement.className.indexOf("checked");
        var brand = $('#brand option:selected') .val();
        var id = $('input[name=id]').val();
        layer.confirm('确认要提交吗？',function(index){
			$.ajax({
				type: 'POST',
				url: "{{url('/service/product_add')}}",
				dataType: 'json',
				data:{id:id,brand:brand,category_id:from_id,product_name:product_name,country:country,weight:weight,original_price:original_price,current_price:current_price,detail:detail,is_comment:is_comment,image:image,_token: "{{csrf_token()}}"},
				success: function(data){
					if(data.status == '0'){
                        ...
                        var obj=document.getElementById("ueditor_0").contentWindow;
                        obj.document.getElementsByTagName("body")[0].innerHTML = '';
	...
	callback: {
		beforeClick: function(treeId, treeNode) {
			...
			if (treeNode.isParent) {
				...
				content = '<button onClick="article_save_submit(\''+treeNode.id+'\');" class="btn btn-primary radius" type="button"><i class="icon-save "></i>保存并提交审核</button>';
        ...
        $(function() {
            ...
            uploader = WebUploader.create({
                ...
                onUploadSuccess: function (file, data, response) {  //一个文件上传成功后的响应事件处理
                    ...
                    inputx.type = "hidden";                                     // 生成input类型
                    inputx.value = data.url;   // 生成input属性value
                    inputx.id = "image";                                         // 赋上id ，当做参数；
                    inputx.className = "image";                                         // 赋上id ，当做参数；
                    inputx.name = "image";                                         // 赋上id ，当做参数；
                    document.getElementById("form-article-add").appendChild(inputx);                             //将按钮附加到父div中去
