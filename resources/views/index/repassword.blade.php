            ...
            <div class="login" style="display: block;">
                ...
                <div class="login-body">
                    <div class="login-style" style="display: block;">
                        <input name="id" class="a"  type="hidden" id="id" value="{{$id}}" placeholder="请输入密码" />
                        <dl><dd><input name="txtUser" class="a"  type="text" id="txtUser" value="" placeholder="请输入验证码" /></dd></dl>
                        <div class="tishi"></div>
                        <dl><dd><input name="txtUser" class="a"  type="password" id="txtUser" value="" placeholder="请输入密码" /></dd></dl>
                        <div class="tishi"></div>
                        <dl><dd><input name="txtUser" class="a"  type="password" id="txtUser" value="" placeholder="请再次输入密码" /></dd></dl>
                        <div class="tishi"></div>

                        <button onClick="passwordSub()" id="logbtn" style="outline:none">提交</button>
                    </div>
                </div>
            </div>
...
<script>
    function passwordSub() {
        ...
        var code = document.getElementsByName("txtUser")[0].value;
        if (code == "") {
            document.getElementsByClassName("tishi")[0].innerHTML = '请输入验证码';
            $("#txtUser").focus();
            return;
        }
                ...
                success: function (data) {
                    if(data.password){
                        for(var i=0; i<data.password.length; i++){
                            ...
                            document.getElementsByClassName("tishi")[0].innerHTML += '<span>' + data.password[i] + '</span></br>';
                        }
                    }
